...
get_ifi_info(int family, int doaliases)
{
	...
	struct ifconf		ifc;
	struct ifreq		*ifr, ifrcopy, ifrindex, ifrhaddr;
...
/* include get_ifi_info2 */
	for (ptr = buf; ptr < buf + ifc.ifc_len; ) {
		...
		sdlname = ifr->ifr_name;
		idx = ifrindex.ifr_ifindex;
#endif

#if defined SIOCGIFHWADDR && defined IFHWADDRLEN
		ifrhaddr = *ifr;
		if (ioctl(sockfd, SIOCGIFHWADDR, &ifrhaddr) == -1) {
			perror("ioctl error");
			exit(1);
		}
		haddr = ifrhaddr.ifr_hwaddr.sa_data;
		hlen = IFHWADDRLEN;
#endif
