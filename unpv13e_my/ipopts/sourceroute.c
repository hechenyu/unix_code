...
{
	if ((optr = malloc(44)) == NULL) {		/* NOP, code, len, ptr, up to 10 addresses */
		perror("malloc error");
		exit(1);
	}
...
inet_srcrt_add(char *hostptr)
{
	...
	if (ocnt > 9) {
		fprintf(stderr, "too many source routes with: %s\n", hostptr);
		exit(1);
	}

	if ((ai = host_serv(hostptr, NULL, AF_INET, 0, &n)) == NULL) {
		fprintf(stderr, "host_serv error for %s, %s: %s",
				 (hostptr == NULL) ? "(no hostname)" : hostptr,
				 "(no service name)",
				 gai_strerror(n));
		exit(1);
	}
